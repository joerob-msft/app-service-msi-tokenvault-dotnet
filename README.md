...
# Using "Token Vault" from an App Service Site (with a Managed Service Identity)

## Prerequisites
To deploy this sample, you need the following:
1. An Azure subscription to create App Service and Token Vault resources.
2. Completion of the App Service "Token Vault" Private Preview onboarding process (Please contact our team for further instructions).
...
1. Same prerequisite items as above, an Azure subscription and going through the "Token Vault" onboarding process.
...
In the Web.config file, change the tokenvaultname in the key tokenResourceUrl to the one you just created. Replace **TokenVaultName** with the name of your Token Vault, where `[token-vault-name]` is the name of your token vault, that you creatde in the previous step.

```xml   
<add key="tokenResourceUrl" value="https://[token-vault-name].westcentralus.tokenvault.azure.net/services/dropbox/tokens/sampleToken" />
```
